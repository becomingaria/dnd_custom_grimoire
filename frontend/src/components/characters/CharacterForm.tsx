...
import type { CreateCharacterInput, Character } from "@/types/character"
...
const labelClass =
    "block font-rajdhani text-sm font-semibold uppercase tracking-wider text-grimoire-text-muted mb-1"
...
export default function CharacterForm({
    ...
    const {
        ...
        defaultValues: initial
            ? {
                  ...
                  level: initial.level,
                  spellcastingAbility: initial.spellcastingAbility as
                      | "INT"
                      | "WIS"
                      | "CHA",
        ...
        if (watchedClass) {
            const ability =
                classSpellcastingAbility[
                    watchedClass as keyof typeof classSpellcastingAbility
                ]
            if (ability) {
                setValue(
                    "spellcastingAbility",
                    ability as "INT" | "WIS" | "CHA",
                )
            }
        }
    ...
    const error = (key: keyof FormValues) =>
        errors[key] ? (
            <p className='mt-1 font-rajdhani text-xs text-red-400'>
                {errors[key]?.message}
            </p>
        ) : null
