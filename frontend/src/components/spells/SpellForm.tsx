...
const spellSchema = z.object({
  ...
  components:         z.array(z.enum(['V', 'S', 'M'])).min(1, 'At least one component'),
...
const labelClass = 'block font-rajdhani text-xs font-semibold uppercase tracking-widest text-grimoire-text-faint mb-1.5';
const errorClass = 'mt-1 font-rajdhani text-xs text-grimoire-danger';

export default function SpellForm({ initial, onSubmit, onCancel, isLoading }: SpellFormProps) {
  const {
    register,
    handleSubmit,
    watch,
    ...
    defaultValues: {
      ...
      components:         (initial?.components as ('V'|'S'|'M')[]) ?? ['V', 'S'],
  ...
  function toggleComponent(comp: 'V' | 'S' | 'M') {
    ...
    setValue(
      'components',
      current.includes(comp) ? current.filter((c) => c !== comp) : [...current, comp]
    );
  }
  ...
  const handleFormSubmit = (data: SpellFormData) => {
    const input: CreateSpellInput = {
      ...
      damageType:         (data.damageType as CreateSpellInput['damageType']) ?? null,
    };
    void onSubmit(input);
  };
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2">
          ...
          <input className={fieldClass} placeholder="e.g. Arcane Bolt" {...register('name')} />
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input className={fieldClass} placeholder="1 action" {...register('castingTime')} />
        </div>
        <div>
          <label className={labelClass}>Range</label>
          <input className={fieldClass} placeholder="60 feet" {...register('range')} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input className={fieldClass} placeholder="Instantaneous" {...register('duration')} />
        </div>
        <div>
          <label className={labelClass}>Source</label>
          <input className={fieldClass} placeholder="Homebrew" {...register('source')} />
        </div>
      </div>
      ...
      <div>
        ...
        {errors.components && <p className={errorClass}>{errors.components.message}</p>}
      </div>
        ...
        {(['concentration', 'ritual'] as const).map((field) => (
          <label key={field} className="flex cursor-pointer items-center gap-2.5">
            <input
              ...
              {...register(field)}
            />
            <span className="font-rajdhani text-sm capitalize text-grimoire-text-muted">{field}</span>
          </label>
        ))}
      </div>
      ...
      {/* Higher levels */}
      <div>
        <label className={labelClass}>At Higher Levels (optional)</label>
        <textarea
      ...
      <div>
        ...
        {errors.classes && <p className={errorClass}>{errors.classes.message}</p>}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input className={fieldClass} placeholder="fire, cold, force…" {...register('damageType')} />
        </div>
        <div>
          <label className={labelClass}>Tags (comma-separated)</label>
          <input className={fieldClass} placeholder="damage, aoe, fire" {...register('tags')} />
        </div>
      </div>
