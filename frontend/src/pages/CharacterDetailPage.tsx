...
export default function CharacterDetailPage() {
  ...
  async function handleUpdate(data: UpdateCharacterInput) {
    ...
    await updateCharacter.mutateAsync({ characterId, data });
    setShowEdit(false);
  }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 py-8">
      ...
      >
        <div className="border-b border-grimoire-border p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="font-cinzel text-3xl font-bold text-grimoire-text-base">{character.name}</h1>
          ...
          <div className="mt-5 grid grid-cols-4 gap-3">
            {[
              { icon: Shield, label: 'Level', value: character.level, color: 'text-grimoire-primary-light' },
              { icon: BookOpen, label: 'Known', value: character.knownSpellIds.length, color: 'text-grimoire-accent' },
              { icon: Star, label: 'Prepared', value: character.preparedSpellIds.length, color: 'text-yellow-400' },
              {
                icon: Shield,
                label: 'Save DC',
              ...
              <div key={label} className="flex flex-col items-center justify-center rounded-xl border border-grimoire-border/60 bg-grimoire-surface/50 p-3 text-center">
                <Icon size={14} className={`mb-1 ${color}`} />
                <span className={`font-mono text-2xl font-bold ${color}`}>{value}</span>
                <span className="font-rajdhani text-[10px] uppercase tracking-widest text-grimoire-text-faint">{label}</span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-4 font-rajdhani text-sm text-grimoire-text-faint">
            <span>
              Spellcasting: <span className="text-grimoire-text-muted">
                {spellcastingAbilityLabel[character.spellcastingAbility as keyof typeof spellcastingAbilityLabel]}
              </span>
            </span>
            {character.spellAttackBonus !== undefined && (
              <span>
                Attack Bonus: <span className="text-grimoire-text-muted">+{character.spellAttackBonus}</span>
              </span>
            )}
          </div>
      ...
      {/* Edit Modal */}
      <Modal isOpen={showEdit} onClose={() => setShowEdit(false)} title="Edit Character">
        <CharacterForm
          initial={character}
      ...
      <Modal isOpen={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} title="Delete Character">
        <div className="space-y-4">
          <p className="font-rajdhani text-grimoire-text-muted">
            Are you sure you want to delete <strong className="text-grimoire-text-base">{character.name}</strong>? This cannot be undone.
          </p>
          <div className="flex justify-end gap-3">
            ...
            <motion.button
              ...
              whileHover={{ scale: deleteCharacter.isPending ? 1 : 1.03 }}
              whileTap={{ scale: deleteCharacter.isPending ? 1 : 0.97 }}
            >
