...
export const charactersApi = {
  ...
  delete: (characterId: string) =>
    apiClient.delete<{ message: string; characterId: string }>(`/characters/${characterId}`),

  // ─── Spell linking helpers ────────────────────────────────────────────────
  addKnownSpell: (characterId: string, character: Character, spellId: string) =>
    apiClient.put<Character>(`/characters/${characterId}`, {
      knownSpellIds: [...new Set([...character.knownSpellIds, spellId])],
    }),

  removeKnownSpell: (characterId: string, character: Character, spellId: string) =>
    apiClient.put<Character>(`/characters/${characterId}`, {
  ...
  togglePreparedSpell: (characterId: string, character: Character, spellId: string) => {
