...
export const spellsApi = {
  list: (filters?: SpellFilters) => {
    ...
    if (filters?.homebrew !== undefined) params.set('homebrew', String(filters.homebrew));
    if (filters?.class)               params.set('class', filters.class);

    const query = params.toString();
    return apiClient.get<SpellsResponse>(`/spells${query ? `?${query}` : ''}`);
  },

  get: (spellId: string) =>
    apiClient.get<Spell>(`/spells/${spellId}`),
  ...
  delete: (spellId: string) =>
    apiClient.delete<{ message: string; spellId: string }>(`/spells/${spellId}`),
};
